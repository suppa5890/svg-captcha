...
const getTextMultipleLines = function (textForMultipleLines, width, height, options) {
	let j = -1;
	const lines = textForMultipleLines.length;

	const out = [];
	const heightToConsider = height / lines;

	while (++j < lines) {
		const text = textForMultipleLines[j];
		const len = text.length;
		const spacing = (width - 2) / (len + 1);
		const min = options.inverse ? 10 : 0;
		const max = options.inverse ? 14 : 4;
		...
		while (++i < len) {
			const x = spacing * (i + 1);
			const y = (j * heightToConsider) + (heightToConsider / 2);
			const charPath = chToPath(text[i], Object.assign({x, y}, options));
...
const createCaptchaCustom = function (textLines, options) {
	...
	const paths =
		[].concat(getLineNoise(width, height, options))
			.concat(getTextMultipleLines(textLines, width, height, options))
...
module.exports.generateRandomMathText = random.mathExpr;
module.exports.create = create;
module.exports.createCaptchaCustom = createCaptchaCustom;
